...
public class PortInSubmitLSRFromCSROnlyRequestTest extends BaseTest {
    @Test(priority = 20)
    public void testPortInSubmitLSRFromCSROnlyRequest() throws IOException, InterruptedException {
        logger = extent.startTest("Test PortIn Submit LSR from CSR Only Request");
        ...
        if(portInPage.errorMessage()==null){
            ...
            portInPage.clickOnActions();
            portInPage.clickOnSubmitLSR();
            projectID = portInPage.enterProjectID();
            logger.log(LogStatus.PASS,"PortIn ProjectID: "+projectID);
            portInPage.selectHandledBy();
            portInPage.clickOnCreate();
            dashboardPage.search(projectID);
            logger.log(LogStatus.PASS,"Searched with Project ID :"+projectID);
            verifyElement("LSR Submitted",portInPage.getStatus());
            dashboardPage.compareAndOpenId(projectID,"ProjectId");
            BasePage.verifyTitle(dashboardPage.getPageTitle());
